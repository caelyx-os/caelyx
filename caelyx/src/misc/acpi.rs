...
use uacpi::{
    ...
    uacpi_initialize, uacpi_interrupt_handler, uacpi_io_addr, uacpi_log_level,
    uacpi_log_level_UACPI_LOG_DEBUG, uacpi_log_level_UACPI_LOG_ERROR,
...
pub extern "C" fn uacpi_kernel_map(
    ...
    let first_virt_page = virt_page_alloc::allocate(page_count as usize)
        .expect("Could not allocate virtual pages for uACPI");

    for i in 0..page_count {
        vmm::map(
            first_phys_page + i * 4096,
            first_virt_page + i * 4096,
            false,
            true,
            false,
            false,
...
pub extern "C" fn uacpi_kernel_unmap(_addr: *mut ::core::ffi::c_void, _len: uacpi_size) {
    ...
    virt_page_alloc::free(
        align_ptr_down(_addr as *const u8, 4096),
        page_count as usize,
    );
}
...
pub extern "C" fn uacpi_kernel_log(_arg1: uacpi_log_level, _arg2: *const uacpi_char) {
    ...
    let _str = unsafe { core::str::from_raw_parts(ptr, len) }
        .strip_suffix("\n")
        .unwrap_or("");
    match _arg1 {
        ...
        #[allow(non_upper_case_globals)]
        uacpi_log_level_UACPI_LOG_INFO => info!("uACPI: {_str}"),
        _ => unreachable!(),
    }
}
